#  Saas DataVenda - Análise de vendas

# Objetivo
Criar um Saas de análise de vendas de alta precisão, para e-commerces em fase de crescimento, com um sistema de inteligência artificial, gerando relatórios e dashboards financeiros e análise de produtos destacando os produtos mais vendidos da empresa.

# Funcionalidades
- Análise automática de produtos mais vendidos 
- Cálculo automático de retorno sobre o investimento 
- Geração automática de relatórios e dashboards
- Geração automática de planilhas por cliente 
- Gráfico em tempo real de evolução de vendas

  # Tecnologias a serem utilizadas
  - Python (Para a lógica principal e funcionamento do Saas)
  - Supabase e SQL (Para o Backend e Banco de dados)
  - Docker (Para empacotamento da aplicação e suas dependências)
  - Render (Para hospedagem)
  - Javascript, HTML e CSS (Para interface Web)
